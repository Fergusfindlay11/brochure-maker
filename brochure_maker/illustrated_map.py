...
class MercatorProjection:
    ...
    def __init__(
        ...
        self.x_scale = usable_w / (self.east - self.west)
        self.y_scale = usable_h / (self.north - self.south)
...
# ──────────────────────────────────────────────
#  Geometry Helpers: Simplification & Filtering
# ──────────────────────────────────────────────
...
def _snap_to_grid(x: float, y: float, grid: float = 1.5) -> tuple[float, float]:
...
def _way_pixel_length(coords: list, proj: MercatorProjection) -> float:
    """Total pixel-length of a way."""
    total = 0.0
...
def _way_pixel_length_fast(pts: list[tuple[float, float]]) -> float:
    """Total pixel-length from pre-projected points."""
    total = 0.0
    for i in range(len(pts) - 1):
        dx = pts[i + 1][0] - pts[i][0]
        dy = pts[i + 1][1] - pts[i][1]
...
def _way_midpoint_fast(pts: list[tuple[float, float]]) -> tuple[float, float]:
...
def _way_angle_fast(pts: list[tuple[float, float]]) -> float:
    """Rotation angle from pre-projected points."""
    n = len(pts)
...
def _way_midpoint(coords: list, proj: MercatorProjection) -> tuple[float, float]:
    """Pixel midpoint of a way."""
    pts = [proj.project(c[0], c[1]) for c in coords]
    n = len(pts)
    mid = n // 2
    if mid >= n:
...
def _way_angle_at_midpoint(coords: list, proj: MercatorProjection) -> float:
    """Rotation angle (degrees) for a label at the midpoint of a way."""
    pts = [proj.project(c[0], c[1]) for c in coords]
    n = len(pts)
    mid = n // 2
    if mid >= n - 1:
...
def _points_to_path_d(pts: list[tuple[float, float]]) -> str:
...
def _coords_to_path_d(coords: list, proj: MercatorProjection) -> str:
...
def _coords_to_polygon_points(coords: list, proj: MercatorProjection) -> str:
...
def _polygon_centroid(coords: list, proj: MercatorProjection) -> tuple[float, float]:
