...
# ──────────────────────────────────────────────
#  Projection (same as illustrated_map.py)
# ──────────────────────────────────────────────

class MercatorProjection:
    """Equirectangular lat/lon -> pixel projection."""
...
def _snap(x: float, y: float, grid: float = 1.5) -> tuple[float, float]:
...
def _way_pixel_length(pts: list[tuple[float, float]]) -> float:
    total = 0.0
...
def _way_midpoint(pts: list[tuple[float, float]]) -> tuple[float, float]:
    n = len(pts)
    mid = min(n // 2, n - 1)
    return pts[mid]


def _way_angle(pts: list[tuple[float, float]]) -> float:
    n = len(pts)
...
def _polygon_points_str(coords: list[tuple[float, float]], proj: MercatorProjection) -> str:
    pts = []
...
def _polygon_centroid(coords: list[tuple[float, float]], proj: MercatorProjection) -> tuple[float, float]:
